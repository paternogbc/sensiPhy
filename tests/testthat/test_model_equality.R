...
###: test samp_phylm:
test_that("mod.0 samp_phylm is equal to phylolm", {
    set.seed(2468)
    tree <- rtree(50)
...
test_that("mod.0 samp_phyloglm is equal to phyloglm", {
    ...
    mod.samp <- samp_phyglm(bin_trait1~pred,data = dat,phy = tree,track = F,times = 1)
    expect_equal(summary.phyloglm(original)$coefficients,
                 mod.samp$full.model.estimates$coef)
})

###: test influ_phylm:
test_that("mod.0 influ_phylm is equal to phylolm", {
    set.seed(2468)
    tree <- rtree(50)
    pred<- rTraitCont(tree,root.value=0,sigma=1,model="BM")
    cont_trait1 <- pred + rTraitCont(tree,model="BM",sigma=0.1)
    ...
    dat<-data.frame(pred,cont_trait1,bin_trait1)
    original <- phylolm(cont_trait1 ~ pred,phy = tree,data = dat,model = "lambda")
    mod.samp <- influ_phylm(cont_trait1 ~ pred,phy = tree,data = dat,model = "lambda",
                             track = F,times = 1)
    expect_equal(summary.phylolm(original)$coefficients,
                 mod.samp$full.model.estimates$coef)
})
...
###: test influ_phyglm:
test_that("mod.0 influ_phyglm is equal to phyglm", {
    set.seed(2468)
    tree <- rtree(50)
    pred<- rTraitCont(tree,root.value=0,sigma=1,model="BM")
    cont_trait1 <- pred + rTraitCont(tree,model="BM",sigma=0.1)
    ...
    dat<-data.frame(pred,cont_trait1,bin_trait1)
    original <- phyloglm(bin_trait1~pred,data = dat,phy = tree)
    mod.samp <- influ_phyglm(bin_trait1~pred,data = dat,phy = tree,track = F)
    expect_equal(summary.phyloglm(original)$coefficients,
                 mod.samp$full.model.estimates$coef)
})
