...
samp <- samp_phylm(log10(range) ~ log10(Mass), data = dat$data, 
...
influ <- influ_phylm(log10(range) ~ log10(Mass), data = dat$data, 
...
```{r, message=F, warning=F, cache=TRUE}
clade <- clade_phylm(log10(range) ~ log10(Mass), data = dat$data, 
...
```{r, message=F, warning=F, cache=TRUE}
trees <- tree_phylm(range ~ Mass, data = dat$data, 
