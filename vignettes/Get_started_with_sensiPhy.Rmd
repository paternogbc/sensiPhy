...
dat <- match_dataphy(log10(range) ~ log10(Mass), data = alien$data, 
                 phy = alien$phy)
```
...
```{r}
mod.0 <- phylolm(log10(range) ~ log10(Mass), data = dat$data, 
...
samp <- samp_phylolm(log10(range) ~ log10(Mass), data = dat$data, 
                 phy = dat$phy[[1]], model = "lambda", track = F)
```

Ploting results:
```{r, fig.height=5.5, fig.width=7.5, warning=F}
sensi_plot(samp)
```
...
influ <- influ_phylolm(log10(range) ~ log10(Mass), data = dat$data, 
...
```{r, message=F, warning=F, cache=TRUE}
clade <- clade_phylolm(log10(range) ~ log10(Mass), data = dat$data, 
...
```{r, message=F, warning=F, cache=TRUE}
trees <- tree_phylolm(range ~ Mass, data = dat$data, 
                 phy = dat$phy, ntree = 100, track = F)
summary(trees)
```
